...
(function (root, factory) {
  ...
  var I18N = {
    ...
    hk: {
      fileOf: function (gb) { return gb + ' GB'; },
      fitYes: '裝得下', fitNo: '裝不下',
      cardLabel: function (gb) { return gb + ' GB 顯卡'; },
      paramLabel: function (b) { return b >= 1000 ? (b / 1000) + ' 萬億' : b + ' B'; },
      bytesNote: function (n, label) {
        return label + '：每個參數佔 ' + n + ' 字節';
      },
      tier: {
        open: '第一檔：標準開源許可',
        conditional: '第二檔：附加條件',
        restricted: '第二檔：限制商用',
        closed: '第三檔：只給 API',
      },
      tierDesc: {
        open: '權重可下載、可商用、可用於訓練新模型，沒有額外門檻。',
        conditional: '權重給了，但許可證里加了自己的條款，商用或再訓練受限。',
        restricted: '權重能拿到，但不允許商業使用，只能研究。',
        closed: '拿不到權重，只能調接口。能力再強也不是開源。',
      },
      sampleLabel: '這一檔裏有：',
      listSep: '、',
      qDownload: '權重能下載嗎？',
      qCommercial: '能商用嗎？',
      qDistill: '能用它的輸出訓練新模型嗎？',
      ansYes: '能', ansNo: '不能', ansLimited: '有條件',
      restart: '重新判斷',
      bizName: {
        ads: '賣廣告', cloud: '賣雲和算力', api: '賣 API 訂閲',
        service: '賣企業定製服務', research: '做研究攢聲譽', model: '直接賣模型',
      },
      bizLogic: {
        ads: '模型是你主業的互補品。互補品越便宜，你的主業越值錢，所以你會希望全世界都能免費拿到好模型。',
        cloud: '開發者拿模型去微調和部署，算力需求最終回到你的雲上。模型免費，算力收費。',
        api: '模型本身就是你在賣的東西。開源等於把自己的商品免費送出去，跟收入結構直接衝突。',
        service: '模型用來建立技術聲譽，錢從私有化部署和定製合同裏來，不靠接口鎖住客户。',
        research: '用最寬鬆的許可換全球範圍的關注度，一次發佈帶來的聲譽遠超同等投入的營銷費用。',
        model: '你的收入直接來自模型授權。把權重放出去，等於取消了自己的收費理由。',
      },
      youLean: '按這個收入結構，你多半會',
      strategy: { open: '傾向開源', closed: '傾向閉源' },
      bizWho: function (who) { return '代表：' + who; },
      abilityOn: '已湧現', abilityOff: '還沒有',
      abilityName: {
        translate: '照着例子續寫', multiling: '跨語言問答', tooljson: '穩定輸出 JSON',
        math: '多步數學推理', roleplay: '長對話裏守住人設', cot: '思維鏈推理',
      },
      emergedCount: function (n, total) { return total + ' 項裏已經出現 ' + n + ' 項'; },
      thresholdAt: function (b) { return '約 ' + b + 'B'; },
      scaleNow: function (b) { return '當前規模：' + (b >= 1000 ? (b / 1000) + ' 萬億' : b + ' B'); },
      costMonthly: function (v) { return '￥' + v.toLocaleString('zh-CN'); },
      costFlagship: '旗艦模型', costSmall: '蒸餾小模型',
      costRatio: function (n) { return '差 ' + n + ' 倍'; },
      costGap: function (v) { return '每月多付 ￥' + v.toLocaleString('zh-CN'); },
      costBarNote: '兩條的長度比就是成本比。這個比例由單價決定，不隨規模變化——所以拖滑塊時變的是上面的金額，不是條的長短。',
      latency: function (ms) { return ms + ' ms'; },
      dailyCalls: function (n) { return '每天 ' + n.toLocaleString('zh-CN') + ' 次請求'; },
      labelCat: '貓', labelDog: '狗', labelRabbit: '兔', labelCar: '汽車',
      hardLabel: '硬標籤', softLabel: '軟標籤',
      tempNow: function (t) { return 'T = ' + t; },
      infoLabel: '學生能學到的資訊量',
      tempHintLow: '分佈很尖，幾乎只告訴學生「是貓」，跟硬標籤差不多。',
      tempHintMid: '分佈軟下來了，學生開始知道貓比較像狗、不太像兔。',
      tempHintHigh: '太平了，各類差異被抹掉，噪音開始蓋過有用資訊。',
      pickAtLeast: '再選一個模型，才能做交叉驗證',
      verdictSame: '這三個都從同一個底座蒸餾而來。它們答錯的時候，會錯得一模一樣，交叉驗證在這裏給不了你任何保障。',
      verdictMixed: '血緣裏有不同的底座，這次交叉驗證才算數。',
      traceBtn: '追溯血緣',
      lineageName: {
        a: '星塵助手', b: '靈犀對話', c: '明析問答',
        d: '岩層 Chat', e: '海圖助理', f: '自研模型 X',
      },
      lineageVendor: {
        a: '星塵科技', b: '靈犀網絡', c: '明析數據',
        d: '岩層智能', e: '海圖雲', f: '內部團隊',
      },
      pickHint: '挑三個看起來最不相干的產品，然後點下面的按鈕。',
      baseName: { qwen: 'Qwen 底座', llama: 'Llama 底座', independent: '獨立訓練' },
    },
    tw: {
      fileOf: function (gb) { return gb + ' GB'; },
      fitYes: '裝得下', fitNo: '裝不下',
      cardLabel: function (gb) { return gb + ' GB 顯示卡'; },
      paramLabel: function (b) { return b >= 1000 ? (b / 1000) + ' 兆' : b + ' B'; },
      bytesNote: function (n, label) {
        return label + '：每個參數佔 ' + n + ' 位元組';
      },
      tier: {
        open: '第一檔：標準開源許可',
        conditional: '第二檔：附加條件',
      ...
      restart: '重新判斷',
      bizName: {
        ads: '賣廣告', cloud: '賣雲和算力', api: '賣 API 訂閱',
        service: '賣企業客製服務', research: '做研究攢聲譽', model: '直接賣模型',
      },
      bizLogic: {
        ads: '模型是你主業的互補品。互補品越便宜，你的主業越值錢，所以你會希望全世界都能免費拿到好模型。',
        cloud: '開發者拿模型去微調和部署，算力需求最終回到你的雲上。模型免費，算力收費。',
        api: '模型本身就是你在賣的東西。開源等於把自己的商品免費送出去，跟收入結構直接衝突。',
        service: '模型用來建立技術聲譽，錢從私有化部署和客製合同裡來，不靠介面鎖住客戶。',
        research: '用最寬鬆的許可換全球範圍的關注度，一次釋出帶來的聲譽遠超同等投入的行銷費用。',
        model: '你的收入直接來自模型授權。把權重放出去，等於取消了自己的收費理由。',
      },
      youLean: '按這個收入結構，你多半會',
      strategy: { open: '傾向開源', closed: '傾向閉源' },
      ...
      abilityOn: '已湧現', abilityOff: '還沒有',
      abilityName: {
        translate: '照著例子續寫', multiling: '跨語言問答', tooljson: '穩定輸出 JSON',
        math: '多步數學推理', roleplay: '長對話裡守住人設', cot: '思維鏈推理',
      },
      emergedCount: function (n, total) { return total + ' 項裡已經出現 ' + n + ' 項'; },
      thresholdAt: function (b) { return '約 ' + b + 'B'; },
      scaleNow: function (b) { return '當前規模：' + (b >= 1000 ? (b / 1000) + ' 兆' : b + ' B'); },
      costMonthly: function (v) { return '￥' + v.toLocaleString('zh-CN'); },
      costFlagship: '旗艦模型', costSmall: '蒸餾小模型',
      costRatio: function (n) { return '差 ' + n + ' 倍'; },
      costGap: function (v) { return '每月多付 ￥' + v.toLocaleString('zh-CN'); },
      ...
      tempHintHigh: '太平了，各類差異被抹掉，噪音開始蓋過有用資訊。',
      pickAtLeast: '再選一個模型，才能做交叉驗證',
      verdictSame: '這三個都從同一個底座蒸餾而來。它們答錯的時候，會錯得一模一樣，交叉驗證在這裡給不了你任何保障。',
      verdictMixed: '血緣裡有不同的底座，這次交叉驗證才算數。',
      traceBtn: '追溯血緣',
      lineageName: {
        a: '星塵助手', b: '靈犀對話', c: '明析問答',
        d: '岩層 Chat', e: '海圖助理', f: '自研模型 X',
